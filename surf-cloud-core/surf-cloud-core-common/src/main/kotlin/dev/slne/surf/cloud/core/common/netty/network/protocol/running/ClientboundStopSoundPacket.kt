...
@SurfNettyPacket(
    DefaultIds.CLIENTBOUND_STOP_SOUND_PACKET,
    PacketFlow.CLIENTBOUND,
    handlerMode = PacketHandlerMode.DEFAULT
)
class ClientboundStopSoundPacket(val uuid: UUID, val soundStop: SoundStop) : NettyPacket(),
    InternalNettyPacket<RunningClientPacketListener> {

    companion object {
        val STREAM_CODEC = StreamCodec.composite(
            ByteBufCodecs.UUID_CODEC,
            ClientboundStopSoundPacket::uuid,
            ByteBufCodecs.SOUND_STOP_CODEC,
            ClientboundStopSoundPacket::soundStop,
