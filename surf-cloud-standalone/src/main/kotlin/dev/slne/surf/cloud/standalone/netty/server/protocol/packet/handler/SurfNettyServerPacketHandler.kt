...
@Component
@Profile("independent")
class SurfNettyServerPacketHandler(private val surfNettyServer: SurfNettyServer) {
    ...
    fun onClientJoin(packet: ClientJoinNettyPacket?, info: NettyPacketInfo) {
        val server = surfNettyServer.connection.clientTracker.clients.asSequence()
    ...
    private fun handleServerInfo(server: CloudServer, action: CloudServerInfoAction) {
        when (action) {
            CloudServerInfoAction.UPDATE_SERVER_INFO -> {
                val list = surfNettyServer.connection.clientTracker
                val clientSource =
    ...
    private fun broadcastPacket(sender: NettyClientSource, packet: NettyPacket<*>) {
        surfNettyServer.connection.clientTracker.clients
            .filter { it != sender }
